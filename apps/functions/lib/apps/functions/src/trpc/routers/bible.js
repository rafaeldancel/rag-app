...
import { GetBooksInputSchema, GetVotdInputSchema, GetChapterInputSchema, BibleBookSchema, VotdSchema, BibleChapterSchema, DailyPrayerSchema, } from '@repo/shared';
import { ENV } from '../../env';
// ─── Constants ────────────────────────────────────────────────────────────────
...
export const bibleRouter = router({
    ...
    getBooks: publicProcedure
        ...
        return (data.data ?? []).map(b => BibleBookSchema.parse({
            usfm: b.id ?? '',
    ...
    getChapter: publicProcedure
        ...
        const data = (await youversionFetch(`/bibles/${versionId}/passages/${book}.${chapter}?format=html`));
        if (!data.content) {
